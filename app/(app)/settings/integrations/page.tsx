...
type IntegrationKey = 'notion' | 'shopify' | 'stripe' | 'google' | 'slack' | 'github';
    ...
    <div className="flex items-center gap-3">
      ...
      <button
        ...
        className={[
          ...
          checked ? 'border-blue-700/30 bg-blue-700' : 'border-zinc-300 bg-zinc-100',
        ].join(' ')}
      >
        <span
          className={[
    ...
    <div className="rounded-3xl border border-white/60 bg-white/70 p-5 shadow-[0_10px_30px_rgba(15,23,42,0.08)] backdrop-blur">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-4">
          ...
          <div>
            <div className="text-lg font-semibold text-zinc-900">{integration.name}</div>
            <div className="text-sm text-zinc-500">{integration.subtitle}</div>
          </div>
        </div>

        <div className="flex flex-col items-end gap-1">
          <Toggle checked={integration.connected} onChange={(next) => onToggle(integration.key, next)} />
          <div className="text-[11px] text-zinc-500">Last update {integration.lastUpdate}</div>
        </div>
      </div>
      ...
      <div className="mt-6">
        ...
        <div className="mt-1 text-sm text-zinc-600">{integration.userIdValue ?? '—'}</div>
      </div>
...
export default function IntegrationsPage() {
  ...
  <div className="flex items-start justify-between gap-6">
    {/* Left content */}
    <div>
      <h1 className="text-3xl font-semibold text-zinc-900">Integrations</h1>
      <p className="mt-2 max-w-3xl text-zinc-600">
        Connect third-party services to Ser3bellum to monitor uptime, data freshness,
        and operational signals — all from one dashboard.
      </p>

      <div className="mt-4 flex flex-wrap gap-2">
        <BadgePill>
          <button onClick={() => setConnectOpen('stripe')}>Stripe</button>
        </BadgePill>
        <BadgePill>
          <button onClick={() => setConnectOpen('google')}>Google</button>
        </BadgePill>
        <BadgePill>
          <button onClick={() => setConnectOpen('shopify')}>Shopify</button>
        </BadgePill>
        <BadgePill>
          <button onClick={() => setConnectOpen('notion')}>Notion</button>
        </BadgePill>
        <BadgePill>
          <button onClick={() => setConnectOpen('slack')}>Slack</button>
        </BadgePill>
        <BadgePill>
          <button onClick={() => setConnectOpen('github')}>GitHub</button>
        </BadgePill>
    ...
    >
      ...
      {/* ✅ Modal lives here (not inside the grid) */}
      {connectOpen && (
