...
import type { DashboardCardDef, DashboardCardId } from "app/(app)/components/DashboardCards";
...
export function CustomizeDashboardModal({ open, onClose, cards, onSaved }: Props) {
  ...
  const defaultEnabled = useMemo(() => cards.filter((c) => c.defaultEnabled).map((c) => c.id), [cards]);
  const defaultMvp = useMemo(() => (defaultEnabled.length ? defaultEnabled : allIds).slice(0, 5), [defaultEnabled, allIds]);
  const defaultEnabled9 = useMemo(() => (defaultEnabled.length ? defaultEnabled : allIds).slice(0, 9), [defaultEnabled, allIds]);
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
            ...
            <div>
              <div className="flex items-center justify-between">
                <div className="text-sm font-medium text-blue-700">Select your 5MVP</div>

                {/* 5 dots */}
                <div className="flex items-center gap-2" aria-label="MVP selection progress">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <span
                      key={i}
                      className={cn(
                        "h-3 w-3 rounded-full",
                        i < mvpCount ? "bg-emerald-500" : "bg-zinc-200"
                      )}
                    />
                  ))}
                </div>
            ...
            <div className="relative">
              ...
              <div className="flex items-center justify-between">
                <div className="text-sm font-medium text-blue-700">Chose your 9 cards</div>
                <div className="text-xs text-zinc-500">{enabledCount}/9 selected</div>
              </div>
...
function RadioDot({ checked }: { checked: boolean }) {
  return (
    <span
      ...
      aria-hidden
    >
      <span className={cn("h-2.5 w-2.5 rounded-full", checked ? "bg-blue-700" : "bg-transparent")} />
    </span>
  );
}
