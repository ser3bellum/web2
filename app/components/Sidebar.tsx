...
/**
 * Glass system
 * - Hover is more obvious (your request)
...
const glassActive =
  "bg-white/18 border-blue-400/35 ring-1 ring-blue-500/20";
...
export function Sidebar() {
  ...
  const appLinks: NavItem[] = useMemo(
    () => [
      { label: "Refresh Data", href: "/refresh", icon: <RefreshIcon /> },
  ...
  const settingsLinks: NavItem[] = useMemo(
    () => [
      { label: "Track new URL", href: "/settings/track-url", icon: <LinkIcon /> },
      { label: "Manage API Keys", href: "/settings/api-keys", icon: <KeyIcon /> },
    ],
    []
  );
...
const toggleCompany = () => {
  ...
  return (
    <aside className="h-full w-72 bg-white/80 z-50 backdrop-blur-xl border-r border-white/40 shadow-[0_2px_4px_rgba(0,0,0,0.08)]">
      <div className="flex h-screen flex-col px-6 py-6">
        ...
        {/* Company selector (now glass, not a heavy pill) */}
        
        <button
          type="button"
          onClick={toggleCompany}
          className={cn("mb-2 mt-4 w-full rounded-lg px-3 py-2 flex items-center justify-between",
        ...
        >
          ...
          <span className="ml-auto opacity-70 text-slate-700">
            <ChevronIcon className={cn("opacity-80 transition-transform duration-200", companyOpen && "rotate-180")}/>
          </span>
        </button>
        <div
  className={cn(
    "grid transition-[grid-template-rows,opacity] duration-200 ease-out mb-3",
    companyOpen ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0 pointer-events-none"
  )}
>
...
<div className="mt-4 flex flex-col gap-2 px-2">
        ...
        <div
          className={cn(
            ...
            appsOpen ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"
          )}
        >
          <div className="overflow-hidden">
              ...
              {connectedApps.map((name) => (
                <button
                  ...
                  onClick={() => setActiveApp(name)}
                   className={cn(
                  "relative w-full rounded-lg px-3 py-2 text-sm transition-colors text-left",
                  "text-slate-500",
                  "hover:bg-slate-100/60",
                  activeApp === name && "text-slate-600"
                  )}
                  >
  {activeApp === name && (
...
<div className="mt-auto pt-6 gap-1">
        {/* Settings dropdown near bottom */}
        ...
        {/* customize Dashboard Button */}
          <button
             ...
             "w-full rounded-xl h-14 px-3 mt-2",
...
/**
 * Dropdown header
 * - Apps and Settings now consistent
 * - Open state gives subtle brand accent (not a big blue fill)
 */
function DropdownHeader({
  ...
  return (
    ...
    >
      ...
      <ChevronIcon
        className={cn("ml-auto opacity-80 transition-transform duration-200", open && "rotate-180")}
      />
    </button>
  );
}
...
/**
 * Nav rows
 * - Active = left rail (brand blue) + subtle ring/border
 * - Hover = stronger contrast (your request)
 */
